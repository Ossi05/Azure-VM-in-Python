...
VM_NAME = config.get('Linux', 'VM_NAME')
...
print(f"Luodaan virtuaalikonetta")
...
poller = compute_client.virtual_machines.begin_create_or_update(RESOURCE_GROUP_NAME, VM_NAME,
    {
        ...
        "storage_profile": {
            "image_reference": {
                "publisher": config.get('image_reference', 'publisher'),
                "offer": config.get('image_reference', 'offer'),
                "sku": config.get('image_reference', 'sku'),
                "version": config.get('image_reference', 'version')
            }
        },
        "hardware_profile": {
            "vm_size": config.get('hardware_profile', 'vm_size')
        },
        "os_profile": {
...
print(f"ssh {USERNAME}@{ip_address_result.ip_address}")


os.system(f'cmd /k "ssh {USERNAME}@{ip_address_result.ip_address}"')